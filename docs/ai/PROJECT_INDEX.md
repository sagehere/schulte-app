...
这是一个舒尔特方格/专注力训练 Web 应用。用户可在浏览器中完成多种训练（含正念引导音频），记录成绩，登录后同步用户资料、每日任务和训练记录；管理员可管理用户、引导音频、重置密码、删除用户并配置时区。应用还提供 `/u/:identifier` 公开成绩页和 `/health` 健康检查。
...
- `src/routes.js`：JSON API 路由，包括用户、成绩、每日任务、公开数据、引导音频、设置和管理员接口。
- `src/audio-guides.js`：引导 MP3 的首次初始化、名称校验、文件 CRUD 与持久化目录解析。
...
- `public/index.html`：单页前端，包含训练模式（含正念音频）、用户中心、每日任务、成绩、本地存储、后台管理 UI。
- `Dockerfile`：生产镜像构建。
